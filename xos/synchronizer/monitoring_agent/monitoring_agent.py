...
import iperfserver_stats as stats
import threading
import subprocess
import six
import uuid
...
publisher_id='iperfserver_publisher'

@app.route('/monitoring/agent/iperfserver/start',methods=['POST'])
def iperfserver_start_monitoring_agent():
    ...
    try:
        ...
        logging.info("Iperfserver monitoring is enabled")
        return "Iperfserver monitoring is enabled"
    except Exception as e:
            return e.__str__()

@app.route('/monitoring/agent/iperfserver/stop',methods=['POST'])
def openstack_stop():
    global start_publish
    start_publish = False
    logging.info ("Iperfserver monitoring is stopped")
    return "Iperfserver monitoring is stopped"
...
def publish_iperfserver_stats(example_stats):
     global producer
     global keystone_tenant_id, keystone_user_id, publisher_id
     ...
     for k,v in example_stats.iteritems():
          msg = {'event_type': 'cord.'+k,
                 ...
                 'payload': {'counter_name':k,
                             ...
                             'resource_id':'iperfserver',
                             'user_id':keystone_user_id,
                             'tenant_id':keystone_tenant_id
                            }
                }
          producer.publish(msg)
          logging.debug('Publishing iperfserver event: %s', msg)
...
def periodic_publish():
    ...
    publish_iperfserver_stats(resParse)
...
if __name__ == "__main__":
    ...
    logging.info ("Iperfserver monitoring is listening on port 5004")
